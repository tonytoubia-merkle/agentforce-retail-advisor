...
const server = http.createServer((req, res) => {
  ...
  // POST /api/claude/directive — fast UIDirective generation via Claude Haiku (server-side, key hidden)
  if (req.url === '/api/claude/directive' && req.method === 'POST') {
    const chunks = [];
    req.on('data', (c) => chunks.push(c));
    req.on('end', () => {
      try {
        ...
        if (!ANTHROPIC_API_KEY) {
          ...
          res.end(JSON.stringify({ error: 'ANTHROPIC_API_KEY not configured — set it in .env.local' }));
          return;
        }

        const { system, message } = JSON.parse(Buffer.concat(chunks).toString());
        if (!system || !message) {
          res.writeHead(400, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
          res.end(JSON.stringify({ error: 'Missing system or message' }));
          return;
        }
        ...
        const claudeReq = https.request(opts, (claudeRes) => {
          const resChunks = [];
          claudeRes.on('data', (c) => resChunks.push(c));
          claudeRes.on('end', () => {
        ...
        if (!res.headersSent) {
          res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
          res.end(JSON.stringify({ error: 'Directive generation failed' }));
        }
      }
    });
    return;
