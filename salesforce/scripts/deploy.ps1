...
function Deploy-Dir($label, $dir) {
    ...
    if (Test-Path $dir) {
        $sfArgs = @("project", "deploy", "start", "--source-dir", $dir, "--target-org", $Org, "--wait", "20")
        if ($DryRun) { $sfArgs += "--dry-run" }
        sf @sfArgs
    } else {
...
if ($DryRun) {
    Write-Host "  Mode       : DRY RUN (validate only, no changes)" -ForegroundColor Yellow
} else {
...
# PHASE 1 - SCHEMA
# ================================================================
Step 1 "Custom Objects (20 custom objects)"
$customObjects = @(
...
# ================================================================
# PHASE 6 - GENAI PROMPT TEMPLATES
# ================================================================
...
foreach ($ps in $permsets) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  OK - Assigned: $ps" -ForegroundColor Green
    } else {
        Write-Host "  WARN - Could not assign (may need manual assignment): $ps" -ForegroundColor Yellow
    }
}
...
if (Test-Path $agentsDir) {
    ...
    sf @agentArgs
    Write-Host "  OK - Agent deploy attempted. Activate in Setup > Agentforce Agents." -ForegroundColor Green
} else {
    Write-Host "  WARN - agents\ directory not found. Activate agents manually in Setup." -ForegroundColor Yellow
}
...
Write-Host "     Setup > Agentforce Agents > Activate Beauty_Concierge and Clientelling_Copilot"
...
Write-Host ""
Write-Host "  3. Remote Site:"
Write-Host "     Setup > Remote Site Settings > SelfOrgDomain"
Write-Host "     Update the URL to your new org My Domain URL"
Write-Host ""
